...
if (len(sys.argv) == 1):
	...
	for j in range( 1, len(sys.argv)):
		if (sys.argv[j] == "-config"):
			...
			infile = sys.argv[j+1]	
datdir=os.path.dirname(infile)
dat_basename=os.path.basename(infile)
infile=infile.strip()
list_files_to_search=[]
pwd = os.getcwd()
#accelpar=[]
#with open (pwd+'/LOG/parametersfile.txt','r') as parfile:
#	par = parfile.read().splitlines()
#	for arg in par:
#		arg=arg.strip()
#                accelpar.append(arg)
...
periodicity_search_FFT_partial(infile)

print "periodicity_search_FFT function ran for file: ", infile
